...
> Historical session log. Append-only. Do not read this file in normal sessions.
> Read only when explicitly asked to look something up.

---

# Archived 2026-05-03T11:30:12Z

# MMV Handoff Brief
> Context bridge for claude.ai, Claude Code, and Cowork sessions.
> **Last updated:** 2026-05-01 (T-1777320000007 shipped on `main`: headless playthrough trace now surfaces skill modifier resolution, identity tag accumulation, practice credit, and `sets_flag` writes automatically on every `choose` step. Six new step types — `train_skill`, `expect_reaction_text`, `expect_identity_axis`, `expect_practice_credit`, `expect_active_skill_progress`, `expect_flag_set`. Sample script `skill_modifier_smoke.yaml` walks Day 0 morning → Glenn afternoon with `musical_ear` trained, asserts `expect_reaction_text variant=skill_modified contains="harmonic progression"`, `expect_identity_axis risk eq 1`, `expect_flag_set glenn_gave_direction`. **Script PASSES on current main** — engine resolver in `dailyLoop.ts:processChoicesForSkills` correctly annotates `reaction_text` with `reaction_with_skill` when the skill is trained. T-1777400000002's bug therefore lives downstream of the API serve (renderer / response transform layer), not in the engine resolution layer; the smoke script remains the regression gate for whichever layer Monty's network-tab diagnostic identifies. The harness now mirrors three additional production write paths inside `choose()`: `tickPracticeCredit` for `practices_skills`, `bumpLifePressure` for `identity_tags`, and resolution of `skill_modifier` against `player_skills`. New `lastChoiceEffects` snapshot on the harness is the trace surface — read by the executor's auto-trace formatter so `choose` step entries gain `skill_modifier`, `reaction_variant`, `identity_tags`, `identity_axes_bumped`, `practiced_skills`, `practice_credits`, `sets_flag` fields without script-side opt-in. New `src/core/playthrough-runner/README.md` documents the full step-type matrix + the regression-test cookbook for skill-modifier paths. **Verification:** tsc clean; vitest 260 passed / 1 skipped (parity with prior baseline); `npm run playthrough:all` 17 passed / 6 failed (was 16/6 — same six pre-existing day0/glenn fails per T-1777297557482; +1 from `skill_modifier_smoke`). 11 new trace goldens committed (scripts that previously had no committed trace), 7 existing goldens re-baselined with the auto-trace fields. **Out of scope deferred:** content-side `default_text: null` audit on Phase 2 retrofits — `glenn_pastime_paradise.head_to_evening` has `reaction_with_skill` populated but no `reaction_text` (default branch renders nothing), worth a backlog ticket if other choices share the pattern. **Phase 1 of T-1777320000007 brief item #5** (period-stance double-write visibility — counter on `daily_states.period_stance_state` AND PERIOD_STANCE row on `choice_log`) confirmed already covered by `expect_period_stance` + `expect_prior_period_stance`; both surfaces hit by `hallway_friction_challenged.yaml`.)
>
> **Earlier 2026-05-01:** (Track A merge complete — `feature/period-stance-infrastructure` merged to main as **`3f0b420`** (no-ff, 48 commits absorbed). 2026-05-01 verifying playtest against build `e9905bf` on a fresh clean account cleared simple-path session behavior and confirmed three sprint c3d4e fixes: audio first visit (T-1777320000005, "music did start at the beginning"), caps mini-game (T-1777310000004, "The CAPS bug is fixed, although the game is damn hard"), and identity disclaimer (T-1777320000008, text-verifiable). Pre-merge & post-merge parity confirmed: tsc clean, vitest 260/1, playthrough:all 16/6 (the 6 are pre-existing day0/glenn per T-1777297557482). Pushed `752ec06..3f0b420 main -> main`. **Five tickets closed clean** (T-1777320000005, T-1777310000004, T-1777320000008 with playtest evidence; T-1777320000001 skill queue lock-out + T-1777320000003 storylet state reset with proxy evidence — explicit repros deferred to next playtest, fixes shipped 2026-04-28 in `6a11f53`). **T-1777400000001 descoped to non-blocking** (Path 2): simple-path verified clean across two playtests (2026-04-29 + 2026-05-01); deploy-during-play hypothesis remains untested, de-prioritized; reopen-on-recurrence; stays `col_doing` so anomalies #1 (`currentSegment`/`hoursRemaining` null in hydrate) and #2 (Day 3 gameMode flip) don't drop off the board. **Two known issues persist** out of scope for the merge: T-1777400000005 (Attend Classes still bottom — diagnosis written into ticket: 2026-04-30 `ae92a70` fix landed in `/api/routine/activities` route handler that has zero UI consumers; actual data path is `dailyLoop.ts:738-742`, which still uses the unmodified `.order("half_day_cost", ascending: true)` query; Track B Option A is ~15 min); T-1777320000004 (storylets repeat as options — dining-hall reproduction in fresh-account in-session play decoupled it from session corruption; remains its own engine bug, likely content-side per pattern of consistent reproduction across accounts). **Four new tickets** filed by claude-pm from the playtest: T-1777665600001 (Room 214 audio re-plays on /play remount), T-1777665600002 (ARPANET Terminal grayed in weekly plan after first visit — bug-or-design spike), T-1777665600003 (Day 5–7 storylet exercising trained skill), T-1777665600004 (Western Civ note-keeper micro-choice missing follow-up). **Branch retention:** `feature/period-stance-infrastructure` not yet deleted; deferred 24h for rollback safety, deletes 2026-05-02 if no rollback signal. **Downstream unblocked:** T-1776329282001 (period friction content — 6 social beats + Jordan + physical beats) is now actively-buildable.)
>
> **Earlier 2026-04-30:** (sprint c3d4e bug-cluster polish — five UI/polish bugs from the 2026-04-27 / 2026-04-29 playtests cleared on `feature/period-stance-infrastructure`. Five commits, one per fix: `c16f71b` T-1777320000008 (identity-selector disclaimer rewritten — replaces the aspirational "won't change the shape of the story" copy with explicit "doesn't change content yet, all paths play as cis het male" notice; selection still saves to `characters.identity_*`; `welcome/page.tsx` carries a TODO for removal once period_stance content gates by identity); `ae92a70` T-1777400000005 ("Attend Classes" reordering — GET /api/routine/activities now sorts by category priority [academic→work→creative/physical→social→practical] then half_day_cost DESC then alphabetical, instead of half_day_cost ASC. Pure route-handler sort, no DB schema change); `b986099` T-1777320000005 (Room 214 first-visit audio — replaced the play-then-fallback dance in `useStoryletAudio` with a module-level interaction gate. Capture-phase listeners on click/keydown/touchstart/pointerdown flip a single global flag; every play() awaits the flag instead of relying on Chrome rejecting the autoplay-blocked promise loud enough to trigger the catch handler. SSR-safe via `typeof window` guards); `1eeaaf6` T-1777310000004 (caps mini-game slider — `startAiming` now sets `phaseRef.current = "aiming"` synchronously before `requestAnimationFrame(meterLoop)`. Prior race: setTimeout-driven second call left the rAF callback running before `useEffect` could update phaseRef from the post-shot "scored"/"missed" value, so meterLoop bailed early without re-queuing. Single-line ref-sync inside the existing useCallback); `04fe688` T-1777320000006 (sleep button flicker — `handleSleep` now awaits the daily-run refetch BEFORE clearing `sleepCardDone`. The reverse order let the SleepCard visibility condition re-evaluate against pre-advance dayState [night, 0h] in the gap between optimistic clear and refetch landing, briefly re-rendering the button. No new state machine, just an `await` reorder). Verification per commit: tsc clean, vitest 260/1 (was 246 — gained tests since last HANDOFF), playthrough:all 16/6 (matches today's baseline; the 6 are the same pre-existing day0/glenn scripts T-1777297557482). Branch now 47 commits ahead of main (5 from this sprint plus 8 from earlier 2026-04-30 work — NPC name coloring, Glenn→Terminal scheduler fix, content-rules docs). Merge gate (Phase 1–4 playtest with `[STATE]` captures attached to T-1777400000001) unchanged — these were polish fixes for the next external playtester, not gate work. Out of scope this sprint: T-1777310000005 snake mini-game (deferred), Step 7/8 cleanup, Sentry follow-up.)
>
> **Earlier 2026-04-30:** (T-1777400000001 Phase 2 observability layer shipped — `@sentry/nextjs` v10.51.0 wired with `[STATE]` log entries auto-emitted as Sentry breadcrumbs. Errors in preview deploys now ship automatically with the trail of state mutations attached, no manual capture needed. Four commits on `feature/period-stance-infrastructure`: `1a1f24b` (Sentry feature: SDK + breadcrumb wiring + setUser + example page), `72fad10` (force webpack build via `next build --webpack` for Sentry source-map plugin; replace deprecated `disableLogger` with new nested `webpack.treeshake.removeDebugLogging`), `ad1ec94` (drop legacy `NEXT_DISABLE_TURBOPACK=1 NEXT_TURBO=0 TURBOPACK=0` env vars from vercel.json — they no-op in Next.js 16 and conflict with `--webpack` flag), `f898fe9` (collateral: Next.js 15+ async params migration on two pre-existing `[id]` pages — `(admin)/admin/storylets/[id]` and `(player)/theory/[id]` — exposed when the bundler switch from Turbopack to webpack revealed stricter type generation). Five Vercel env vars set across Preview-branch + Production scopes: `NEXT_PUBLIC_SENTRY_DSN`, `SENTRY_DSN`, `SENTRY_AUTH_TOKEN` (sensitive, `org:ci` scope), `SENTRY_ORG=massdigi`, `SENTRY_PROJECT=mmv`. `next.config.ts` wrapped with `withSentryConfig`; new files `instrumentation.ts` + `instrumentation-client.ts` + `sentry.{server,edge}.config.ts` at root; `src/app/sentry-example-page/page.tsx` deliberate-error route. End-to-end verified at `https://mmv-17pd2mqjp-monty-sharmas-projects-10b52ab3.vercel.app/sentry-example-page` — Sentry issue `MMV-1` captured with `environment: preview`, `release: f898fe9ae12567994acc739b8898b61b4ec43a81`, source maps uploaded (build log: "Uploaded files to Sentry / Organization: massdigi / Release: f898fe9..."), breadcrumbs panel populated. Verification: tsc clean, vitest 246/1 (parity), playthrough:all 13/7 (parity). Branch now 25 commits ahead of main; merge gate (Phase 1–4 playtest with state captures attached to T-1777400000001) unchanged but the diagnostic surface is now passive auto-capture instead of manual.)
>
> **Earlier 2026-04-29:** (T-1777400000001 Phase 1 shipped — `[STATE]` instrumentation across six state-mutation surfaces (day-advance, both reset routes, tracks/resolve, choice_log writers, walk-state, session-restore) plus the `setDailyState` mid-session mutations and the `handleMicroEffects` micro-choice path. New shared helper `src/lib/stateLog.ts` emits a console line + sessionStorage ring buffer at `mmv:state-log` (cap 200, surfaced as `window.__stateLog`). Build SHA tagged on every entry via existing `NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA` plumbing. New `docs/PHASE-1-LOG-CAPTURE-INSTRUCTIONS.md` is the paste-ready 3-capture protocol for Monty (pre-exit / post-restart-before-action / post-symptom). New `docs/CODE-BRIEF-2026-04-30-state-persistence.md` is the PM brief that drove the work. Two `PHASE-2-NOTE` triage comments dropped (dev-reset omits `period_stance_state` + `preclusion_gates`; client-side `choice_log` insert in track-storylet flow is fire-and-forget). Single standalone commit `a1c807d` — no fixes bundled per Phase 1 discipline. Verification: tsc clean, vitest 246/1 (parity), playthrough:all 13/7 (parity). Two commits pushed origin `cb10dc2..a1c807d`: `6a11f53` (T-1 queue lock-out + T-3 walk-state persistence; was unpushed) + `a1c807d` (this Phase 1 ship). Vercel preview built in ~43s and is READY at `https://mmv-git-feature-period-s-d596e8-monty-sharmas-projects-10b52ab3.vercel.app` — ready for Monty's capture-and-reproduce against the instrumented build. **Phase 2 (log analysis + targeted fix) is pending the captures.** Branch now 21 commits ahead of main; merge gate (Phase 1–4 playtest) unchanged but the diagnostic surface to clear it is now live.)
>
> **Earlier 2026-04-28:** (PATH B executed — repo + Kanban relocated out of iCloud-synced `~/Documents/` to `~/Projects/`, replacing PATH A's `xattr` band-aid with the structural fix. T-1777382919259 closed. Repo now at `~/Projects/V16MMV/mmv`; Kanban at `~/Projects/MMV/_assets/MMV_Docs/Kanban data/`. Six worktrees repaired (4 internal, 1 external `/private/tmp/mmv-signal-hunt`, 1 main). `.claude/settings.local.json` updated (101 absolute paths, sed-replaced; gitignored, local-only). Path-reference sweep across 9 repo files + 1 Kanban ticket; bonus runbook count-drift fixes. Verification at new home all green: `git fsck --full` 1.5s (was 22-min hang), `git fetch origin` clean, `npx tsc --noEmit` clean, vitest 246 passed / 1 skipped (parity), `npm run playthrough:all` 13/7 (parity — the 7 are pre-existing day0/glenn, T-1777297557482), `npx tsx scripts/audit-closed-tickets.ts` exit 0. Two commits on `feature/period-stance-infrastructure`: `49846d5` (path refs) + the HANDOFF/DECISIONS commit landing then. Branch was then 19 commits ahead of main.)
>
> **Earlier 2026-04-28:** (git-infrastructure repair before Phase 1–4 playtest — see Recent Decisions row 2026-04-28 PATH A for full detail. TL;DR: iCloud Documents sync had been silently duplicating `.git/` files, breaking `git fetch`/`fsck`. Cleaned up `.git/index 2` and `.git/refs/remotes/origin/main 2`; set `com.apple.fileprovider.ignore#P` xattr on `.git` to prevent recurrence. Pushed 8 commits: `feature/period-stance-infrastructure` origin tip `60f8a2c` → `9ce0435`. Branch was then 0 unpushed, 17 commits ahead of main, gated only on the playtest. Wider iCloud damage in `.next/` and `node_modules/` was documented but out of scope for that session — handled in PATH B today.)
>
> **2026-04-27:** (sprint b2c3d closer — five tickets shipped: **T-1777300000003** MAPPING regen close (auto-regenerated by Kanban Pro on project load); **T-1777055123000** Dana→Scott visible-prose scrub via migration `20260427100000_dana_to_scott_visible_prose.sql` (idempotent, two node-text strings patched, L4 playthrough re-run PASS 30/30 in 18.9s, one out-of-scope micro-choice label `commit_dana.label` flagged for follow-up); **T-1777300000007** "Branches in flight" section added near top of HANDOFF + SOP.md/CLAUDE.md/end-session protocol updated to reference it (time_skill verified merged via `git rev-list --count main..time_skill`=0 and removed); **T-1777215600002** node-usage review re-executed against live DB with per-node verdicts in `docs/L1-L5-NODE-USAGE-REVIEW-2026-04-27.md` (all five landmarks PASS; one period-detail finding filed as T-1777321300001: Springsteen "Born in the U.S.A." poster on `scott_notices.alone_in_214` is anachronistic for 1983); **T-1777215600100** ticket-vs-disk audit step shipped — `scripts/audit-closed-tickets.ts` walks col_done tickets, regex-extracts file refs, exits 1 on findings. Retroactive sweep on full set: 125 scanned, 50 refs extracted, 1 false positive (T-1776329281200 "replaces planned `docs/RELEASE-TIMELINE.md`"), 0 real staleness. SOP.md drift-check + end-session sections updated; protocol documented in `docs/AUDIT-PATTERN.md`. Five commits on `feature/period-stance-infrastructure`: `013b73e`, `1ec824d`, `2ad977a`, `d7230a6` plus prior `60f8a2c`. **Critical for c3d4e: T-1777300000001** — period-stance merge prep is gated on monty's Phase 1–4 browser playtest (Issue #10).)
>
...
No branches in flight. `feature/period-stance-infrastructure` merged to main on 2026-05-01 as `3f0b420` (48 commits absorbed); branch retained 24h for rollback safety, scheduled deletion 2026-05-02. `time_skill` is fully merged into main (verified 2026-04-27: `git rev-list --count main..time_skill` = 0; tip `fae2d34` reachable from main; 48 commits behind).
...
| **Assets** | `Projects/MMV/_assets/` (once consolidated) | Cowork | Investor decks, logos, promo videos |
...
### Sentry observability, T-1777400000001 Phase 2 (built 2026-04-30)

Wires automatic capture for the same intermittent state-corruption bug Phase 1 instrumented. Where Phase 1 required Monty to capture logs by hand at the moment a bug fires, Sentry inverts: errors and surrounding context get shipped automatically. Each `[STATE]` log entry is also emitted as a Sentry breadcrumb, so when an error fires anywhere in the app the report ships with the trail of state mutations leading up to it — diagnosis input for T-1777400000001 (and any future intermittent bug) without needing the human in the loop at the moment of failure.

**SDK + configs.** `@sentry/nextjs` v10.51.0 installed via npm. New files at root: [instrumentation.ts](instrumentation.ts) (Next.js 16 server hook, branches by `NEXT_RUNTIME` to import server/edge configs; exports `onRequestError = Sentry.captureRequestError`), [instrumentation-client.ts](instrumentation-client.ts) (client SDK init; exports `onRouterTransitionStart = Sentry.captureRouterTransitionStart` for App Router nav tracing), [sentry.server.config.ts](sentry.server.config.ts), [sentry.edge.config.ts](sentry.edge.config.ts). All configs tag `environment: VERCEL_ENV ?? NODE_ENV` and `release: VERCEL_GIT_COMMIT_SHA` (matches existing `BUILD_SHA` pattern in stateLog.ts). Local dev opt-out: `enabled: NODE_ENV !== "development" || SENTRY_DEV_ENABLE === "true"` (server) / `NEXT_PUBLIC_SENTRY_DEV_ENABLE` (client). Client config also wires `Sentry.consoleLoggingIntegration({ levels: ["error", "warn"] })` for level-error-or-warn capture without an explicit throw.

**Breadcrumb wiring (the load-bearing part).** [src/lib/stateLog.ts](src/lib/stateLog.ts) — new `Sentry.addBreadcrumb({ category: "state.<surface>", message: <action>, data: { userId, sha, ...details }, timestamp: Date.parse(entry.ts)/1000 })` call after the existing console.log + sessionStorage paths. **Existing paths are byte-identical**, per the brief's hard rule. `addBreadcrumb` is a safe no-op when Sentry isn't initialized (e.g., the tsx playthrough CLI runner) so no guards needed.

**User context.** [src/app/(player)/play/page.tsx](src/app/(player)/play/page.tsx) — `Sentry.setUser({ id: bootstrapQuery.data.userId })` after the existing `setUserId(...)` in the bootstrap-success effect. Sentry can now filter "errors for the test user during a playtest" trivially.

**withSentryConfig + bundler fix.** `next.config.ts` wrapped with `withSentryConfig(nextConfig, { org, project, silent: !CI, webpack: { treeshake: { removeDebugLogging: true } } })`. The Sentry webpack plugin handles source-map upload at build time. **Critical wrinkle:** Next.js 16 made Turbopack the default for `next build`; the legacy `NEXT_DISABLE_TURBOPACK=1 NEXT_TURBO=0 TURBOPACK=0` env vars in vercel.json no longer work. First Sentry-enabled build silently ran on Turbopack — source maps were NOT uploaded (build-log forensics caught this). Fix: `next build --webpack` flag in `package.json` `build:webpack` script; legacy env vars removed from vercel.json (Next.js 16 hard-errors with "Multiple bundler flags set" when both old env vars and new flag are present).

**Async-params migration (collateral).** Forcing webpack exposed two pre-existing pages still using the synchronous Next.js <15 `params` shape. Turbopack had been lenient about it. [src/app/(admin)/admin/storylets/[id]/page.tsx](src/app/(admin)/admin/storylets/[id]/page.tsx) and [src/app/(player)/theory/[id]/page.tsx](src/app/(player)/theory/[id]/page.tsx) migrated to `params: Promise<{ id: string }>` + `use(params)` (both are `"use client"` components; theory page's outer wrapper resolves once and passes the resolved object down to the inner `TheoryDetailContent`).

**Vercel env vars.** Set during this session via `vercel env add`. Five vars across Preview-branch (`feature/period-stance-infrastructure`) + Production scopes: `NEXT_PUBLIC_SENTRY_DSN`, `SENTRY_DSN`, `SENTRY_AUTH_TOKEN` (sensitive, organization-token style with `org:ci` scope bundle), `SENTRY_ORG=massdigi`, `SENTRY_PROJECT=mmv`. Branch-scoping matches the existing Supabase env-var convention — saved as feedback memory `feedback_vercel_branch_scoped_envs.md`.

**Deliberate-error test route.** [src/app/sentry-example-page/page.tsx](src/app/sentry-example-page/page.tsx) — single-button page that throws a client-side error tagged with timestamp. Used to verify capture end-to-end.

**Verification (post-deploy).** Hit `https://mmv-17pd2mqjp-monty-sharmas-projects-10b52ab3.vercel.app/sentry-example-page`, click button → Sentry issue `MMV-1` created in [massdigi.sentry.io](https://massdigi.sentry.io/) tagged `environment: preview`, `release: f898fe9ae12567994acc739b8898b61b4ec43a81`, transaction `/sentry-example-page`. Build log confirms source-map upload: `Uploaded files to Sentry / Organization: massdigi / Release: f898fe9... / [hundreds of *.js sourcemap entries with debug IDs]`. Stack traces will show real source paths, not minified `~/abc-123.js` noise.

**Verification (parity with baseline).** `npx tsc --noEmit` clean. `npm run test` 246 passed / 1 skipped (parity). `npm run playthrough:all` 13/7 (parity).

**Out-of-scope follow-ups (non-blocking).**
- (1) Add `src/app/global-error.tsx` with Sentry instrumentation for App Router render errors. Build log warning recommends it; suppress meanwhile with `SENTRY_SUPPRESS_GLOBAL_ERROR_HANDLER_FILE_WARNING=1`.
- (2) Switch Sentry project's "Platform" dropdown from React to Next.js for slightly better dashboard hints (cosmetic, doesn't affect capture behavior).
- (3) Silent state corruption that doesn't throw still won't surface via breadcrumbs alone — Phase 2 diagnosis decides where explicit `Sentry.captureMessage` calls belong after seeing at least one breadcrumb-enriched report.

### `[STATE]` instrumentation, T-1777400000001 Phase 1 (built 2026-04-29)

Diagnosis-first observability pass for the session/restart state-corruption bug. Six fix-and-retry passes had failed to find the root cause; this commit ships the instrumentation that lets Phase 2 read what's happening to state across the lifecycle instead of guessing. Single standalone commit `a1c807d` on `feature/period-stance-infrastructure`. Vercel preview READY at `https://mmv-git-feature-period-s-d596e8-monty-sharmas-projects-10b52ab3.vercel.app`. **No fixes shipped** — six pre-existing fix candidates were spotted while wiring; all deferred to Phase 2 (two captured as `PHASE-2-NOTE:` triage comments, the rest noted in this section's "deferred" list).

**Shared helper.** [src/lib/stateLog.ts](src/lib/stateLog.ts) — single `logState({surface, action, userId?, details?})` export. Emits `console.log("[STATE]", JSON.stringify(entry))` server- and client-side; on the client also pushes to `sessionStorage["mmv:state-log"]` (ring-buffered to 200 entries) and assigns `window.__stateLog` for devtools triage. Build SHA tagged on every entry via the existing `NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA → NEXT_PUBLIC_COMMIT_SHA → "dev"` fallback chain (matches the existing login-page build-stamp pattern).

**Six surfaces wired.**

| # | Surface | File(s) | What's logged |
|---|---------|--------|---------------|
| 1 | Day-advance / segment-advance | `src/app/api/time/advance/route.ts` | request entry, dispatch (segment vs day branch), advanceSegment.before/after/raceLost/error, advanceDay.before/after/raceLost/finalizeDay/createNextDayState/error, player_day_state mirror sync |
| 2 | Reset endpoints | `src/app/api/run/reset/route.ts`, `src/app/api/admin/dev/reset/route.ts` | resetUser.start, priorState, deletes.before/after, dailyStates.before (with `bcfc171Coverage` field naming the segment+hours fields), complete |
| 3 | Storylet resolve | `src/app/api/tracks/resolve/route.ts` | resolveTerminalChoice.start (with canonical dayIndex from daily_states), trackProgress.{stateUpdate, advanceNext, stayActive, complete, activate}, preclusionGates.update, duplicate guard, complete |
| 4 | choice_log writes | `src/app/api/tracks/resolve/route.ts`, `src/lib/play.ts` (PERIOD_STANCE), `src/lib/resources.ts` (RESOURCE_APPLIED), `src/core/playthrough-runner/harness.ts` (verified real-DB inserts at lines 310/334/440, tagged `harness:true` in details to filter from real-player runs) | choiceLog.{storyletResolved, flagSet, periodStance, resourceApplied} |
| 5 | Walk-state sessionStorage | `src/components/play/DialogueNodeView.tsx` | walkState.{read, write, clear, read.error, write.error} with reason field (terminalChoice vs explicit) |
| 6 | Session restoration + setDailyState mid-session | `src/app/(player)/play/page.tsx` | sessionRestore.{bootstrap.success, bootstrap.error, dailyRun.success, identityStance}, hydrate (initial load), microChoice.{start, relationships, lifePressure, periodStance, complete}, terminalResolve.{relationships, lifePressure, skillFlags, preclusionGates, optimisticDayState}, trackStoryletChoice.relationships, resolveTerminalChoice.{clientStart, clientComplete, clientError} |

**Deliverable for Monty.** [docs/PHASE-1-LOG-CAPTURE-INSTRUCTIONS.md](docs/PHASE-1-LOG-CAPTURE-INSTRUCTIONS.md) — paste-ready 3-capture protocol: pre-exit (after a normal Day 1 walk), post-restart-before-action (the most diagnostic frame — do not click), post-symptom (after the bug fires). Each capture is `JSON.stringify(window.__stateLog)` from devtools + the server log's `[STATE]` lines, dropped under `docs/captures/<timestamp>-<phase>.{client.json|server.log}`. The captures dir is in `.gitignore` (local diagnostic dumps don't ship). The PM brief at [docs/CODE-BRIEF-2026-04-30-state-persistence.md](docs/CODE-BRIEF-2026-04-30-state-persistence.md) explains the why.

**`PHASE-2-NOTE` comments dropped (greppable triage list).**
- `[surface 2, dev-reset]` `/api/admin/dev/reset` omits `period_stance_state` and `preclusion_gates` from its `daily_states` reset; `/api/run/reset` includes them. Divergent reset shapes — could matter for the "reset to end of Day 1" symptom on dev-menu-driven resets specifically.
- `[surface 2, dev-reset]` deletes are sequential (await per call) here vs `Promise.all` parallel in run-reset — different atomicity surface.
- `[surface 6b, line 2743 of play/page.tsx]` client-side `choice_log.insert` for REL_DELTA events fires-and-forgets without await, outside the `/api/tracks/resolve` boundary. Under-instrumented vs the server-side `choiceLog.{storyletResolved, flagSet}` events.

**Spotted-but-not-fixed (deferred to Phase 2).** `bcfc171` did cover both reset endpoints (verified by reading both routes — both reset `current_segment`, `hours_remaining`, `hours_committed` alongside `day_index`). If the "restart resets to end of Day 1" symptom recurs against the instrumented build, the bug is *not* in the reset shape — it's either in (a) the on-mount hydrate path patching state from a stale source, (b) `daily_states` ↔ `player_day_state` segment-write disagreement (the mirror update returns success but writes nothing if the `player_day_state` row is missing), or (c) a non-reset code path — the logs will pick.

**Verification (parity with baseline).** `npx tsc --noEmit` clean. `npx vitest run` 246 passed / 1 skipped (was 246/1). `npm run playthrough:all` 13/7 (the 7 failures are pre-existing day0/glenn per backlog T-1777297557482, outside scope). `[STATE]` lines fired in vitest output for `choiceLog.resourceApplied` (allocation tests) and in playthrough output for `choiceLog.storyletResolved`/`flagSet`/`periodStance` with `harness:true` tag.

**Process improvement.** Monty noted that `HANDOFF.md` was stale by six commits when he drafted the brief — making it harder to reason about the failing playtest. Going forward, claude.ai sessions will either (a) get a one-line entry per commit on a branch in flight in `HANDOFF.md` before EOD, or (b) the next Code session-opener summary includes a `git log --since=<last-handoff>` snippet so the PM doesn't have to read the full log. Saved as a feedback memory.
...
- **T-1777400000001 Phase 2 — log capture + targeted fix.** Phase 1 instrumentation deployed today (commit `a1c807d`, Vercel preview READY). Pending: Monty runs the 3-capture protocol from `docs/PHASE-1-LOG-CAPTURE-INSTRUCTIONS.md` against the instrumented build, drops the captures dir on the ticket. Phase 2 session reads the logs (zen `thinkdeep` with the captures as input), identifies the divergence-timestamp + responsible write/read, ships the targeted fix + a regression playthrough that exercises the exit-and-restart sequence.
...
|------|----------|---------|
| 2026-04-29 | **T-1777400000001 Phase 1 shipped: `[STATE]` instrumentation across six surfaces, no fixes, standalone commit `a1c807d`. Phase 2 (log analysis + targeted fix) gated on captures from a fresh playtest run against the Vercel preview.** | Multiple prior sessions had read code, formed a hypothesis, shipped a fix, watched the symptom recur in a new form. PM brief at `docs/CODE-BRIEF-2026-04-30-state-persistence.md` named the discipline that's been missing: observability before code-reading. Phase 1 wires `[STATE]` logging across six state-mutation surfaces — day-advance, both reset routes, tracks/resolve, choice_log writers, walk-state, session-restore — plus the `setDailyState` mid-session mutations and `handleMicroEffects`. Shared helper `src/lib/stateLog.ts` (single `logState({surface, action, userId?, details?})` export). Build SHA tagged via existing `NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA` plumbing. Client logs ring-buffered to `sessionStorage["mmv:state-log"]` (cap 200, mounted as `window.__stateLog`). New paste-ready 3-capture protocol at `docs/PHASE-1-LOG-CAPTURE-INSTRUCTIONS.md` (pre-exit / post-restart-before-action / post-symptom). Two `PHASE-2-NOTE:` triage comments dropped: dev-reset omits `period_stance_state` + `preclusion_gates` from its `daily_states` reset (divergent shape vs run-reset); track-storylet flow client-side `choice_log.insert` for REL_DELTA fires-and-forgets without await. `bcfc171` verified to cover both reset endpoints (current_segment + hours_remaining + hours_committed reset alongside day_index). Single standalone commit `a1c807d` per "no fixes in Phase 1, even if you spot one" discipline. Verification: tsc clean, vitest 246/1 (parity), playthrough:all 13/7 (parity). Pushed origin `cb10dc2..a1c807d` (also picks up unpushed `6a11f53` T-1/T-3 playtest fix). Vercel preview READY in ~43s. **Phase 2 will pick exactly one of**: persistence bug (writes don't land), resume-read bug (reads pull stale data), resume-write bug (mount rebuilds from stale source), sessionStorage contamination, day-advance double-fire, segment transition desync. |
| 2026-04-29 | **HANDOFF refresh discipline: one-line entry per commit on a branch in flight, before EOD; or a `git log --since=<last-handoff>` snippet at the top of next session's opener.** | Today's session opener missed six commits that had landed on `feature/period-stance-infrastructure` since the 2026-04-28 PATH B HANDOFF (`c7a11d50`, `fcfa7ef`, `5c224ee`, `bcfc171`, `6f1723f`, `cb10dc2`, `6a11f53`) — making it harder for the PM to draft an accurate brief and giving Code an incomplete picture of what was already deployed. Going forward: Code updates HANDOFF "Branches in flight" or appends a one-liner per meaningful commit before EOD; or includes `git log --since` of unmentioned commits in the session-opener summary. Saved as a feedback memory so this carries across sessions. |
| 2026-04-29 (commit backfill since 2026-04-28 PATH B) | **`6a11f53` fix(playtest): T-1 queue lock-out, T-3 walk state persistence.** **`cb10dc2` fix: gate SegmentTransitionCard on `!isFetching`, not resolvedTrackStoryletIds (root cause of day-2-evening stick after the explicit-click revert in `6f1723f`).** **`6f1723f` fix: revert SegmentTransitionCard to explicit Continue button (reverts ad97554's 300ms in-card auto-advance, which 00edffc had previously retired for being a stale-closure pattern).** **`bcfc171` fix: reset endpoints must reset segment + hours alongside day_index (both `/api/run/reset` and `/api/admin/dev/reset` now mirror `/api/time/advance` day-rollover semantics — current_segment='morning', hours_remaining=16, hours_committed=0 atomically with day_index=1).** **`5c224ee` fix: gate SegmentTransitionCard auto-advance on resolved beats (key={dayIndex-currentSegment} remount removed firedRef safety; gate on `(resolvedIds.size > 0 || stage === 'complete')` closes the loading-window race).** **`fcfa7ef` content: compress tier-1 skill durations for playtest (DB values, not the env-var-only `NEXT_PUBLIC_SKILL_TIME_SCALE`).** **`c7a11d50` chore: redeploy with Preview Supabase env vars.** | These shipped between the 2026-04-28 PATH B session and 2026-04-29's instrumentation work. Backfilled here because the HANDOFF refresh discipline (above row) requires it. |
| 2026-04-28 (PATH B) | **Repo + Kanban relocated from `~/Documents/` (iCloud-synced) to `~/Projects/` (local-only). T-1777382919259 closed.** | PATH A's xattr was a band-aid; PATH B is the structural fix. Pre-move: nuked `node_modules` (was in known-unhealthy state from iCloud collisions; `.next/` was already absent). Same-volume `mv` (filesystem rename, fast, atomic): `~/Documents/V16MMV/mmv` → `~/Projects/V16MMV/mmv`; `~/Documents/MMV/_assets/MMV_Docs/Kanban data/` → `~/Projects/MMV/_assets/MMV_Docs/Kanban data/`. Worktree repair: 4 internal under `.claude/worktrees/` showed prunable until `git worktree repair <new-paths>` re-anchored their gitdir refs; external `/private/tmp/mmv-signal-hunt` repaired automatically by the main repair pass. (The original "5 worktrees" estimate from yesterday was off by one; actual count is 6 = 4 internal + 1 external + 1 main.) `.claude/settings.local.json` had **101** absolute paths to the old location (yesterday's estimate was ~80) — sed-replaced; file is gitignored, line count unchanged at 401. Path-reference sweep across 9 repo files + 1 Kanban ticket; bonus runbook count-drift fixes (replaced hardcoded "7 commits" phrasing with non-counted equivalents in `docs/PHASE-1-4-PLAYTEST-RUNBOOK.md`). DECISIONS.md lines 6+15 (yesterday's PATH A diagnosis) left as historical record. test_output.json deleted (stale generated artifact). Verification at new home: `git fsck --full` 1.5s (was 22-min iCloud-eviction hang yesterday); `git fetch origin` clean; `npx tsc --noEmit` clean; `npx vitest run` 246 passed / 1 skipped (parity); `npm run playthrough:all` 13/7 (parity — 7 are pre-existing day0/glenn, T-1777297557482); `npx tsx scripts/audit-closed-tickets.ts` exit 0; no iCloud xattrs at new location. Two commits on `feature/period-stance-infrastructure`: `49846d5` (path refs) + this HANDOFF/DECISIONS commit. Branch is now 19 commits ahead of main. **Followup ticket close:** T-1777382919259 to be moved to `col_done` with this entry as evidence. **Followup learning captured in DECISIONS.md:** always grep `.claude/settings.local.json` and `git worktree list` before estimating relocation cost; same-volume `mv` is fast; the harness's cwd check breaks on `mv` of the active session's CWD — recreate a placeholder file at the old path to recover. |
| 2026-04-28 (PATH A) | **Git-infrastructure repair: iCloud Documents sync had been silently corrupting `.git/`. Cleanup + per-folder iCloud exclusion + 8 commits pushed.** | Pre-flight for Phase 1–4 playtest failed with `fatal: bad object refs/remotes/origin/main 2` — the literal `" 2"` suffix is macOS's sync-conflict naming pattern. Confirmed the repo lives inside iCloud's "Desktop & Documents Folders" sync (parent `~/Documents` carries xattr `com.apple.icloud.desktop`). Diagnosed PATH A (in-place cleanup) over PATH B (relocate to `~/Code/`) because the working-tree damage was minor (no working-tree dups outside `.next/`/`node_modules/`) and relocation cost is real (5 worktrees, ~80 hardcoded paths in `.claude/settings.local.json`). Executed: deleted `.git/index 2` and `.git/refs/remotes/origin/main 2` (the bad ref pointed to stale `ab90671` from April 18; the good `main` ref correctly pointed to `752ec06`); ran `git remote prune origin && git fetch origin --prune` clean; set `xattr -w com.apple.fileprovider.ignore#P 1 .git` to keep iCloud out of the ref store going forward. Pushed feature branch: origin tip `60f8a2c` → `9ce0435` (8 commits). Wider iCloud damage discovered along the way: `.next/` cache had ~30 sync-duplicates (deleted, gitignored — Next.js will regenerate); `node_modules/` is full of duplicate scope dirs (`@alloc 2/`, `@babel 2/`, etc.) and at least `playwright/package.json` is partially evicted, which is what blocks `tsc --noEmit` (pre-existing, not from these 8 commits). `git fsck --full` hung 22 min on `.git/objects/5e/3a99...` mmapping an iCloud-evicted blob (call stack: `read_loose_object → map_fd`); was killed and replaced with lighter ref-read verification — `origin/main` reads cleanly (`752ec06`), `main..HEAD = 17`, `HEAD..main = 0`. Vitest 246 passed / 1 skipped — parity with prior green. **Followups for next session:** (a) full environment heal `rm -rf node_modules .next && npm install` once iCloud is excluded from the working tree (or relocate per PATH B); (b) optionally `git fsck --full` once eviction stops blocking it; (c) the playtest itself — the git precondition is now met. |
...
# Archived 2026-05-28 (HANDOFF "Recently merged" trim at NewsNet merge / Gate 2 ship)

These entries were trimmed from `HANDOFF.md` "Recently merged (last 7 days)" when the 2026-05-28 NewsNet merge landed. The 7-day window now starts at 2026-05-21, so everything below moves here.
