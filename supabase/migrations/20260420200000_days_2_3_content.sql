-- Days 2–3 content: 10 new storylets extending academic, belonging, money, roommate tracks.
-- Specs from docs/DAYS-2-3-CONTENT-BRIEF.md. See HANDOFF.md for milestone context.
--
--   academic (5):  western_civ_day1 (d2 morn), reading_or_lounge (d2 eve, collision),
--                  second_morning_class (d3 morn), study_group_forming (d3 aft),
--                  catch_up_or_coast (d3 eve, requires_flag skipped_reading)
--   belonging (3): floor_lunch_day2 (d2 aft), hallway_morning_day3 (d3 morn),
--                  miguel_afternoon_day3 (d3 aft)
--   money (1):     bookstore_line (d2 aft)
--   roommate (1):  roommate_evening_day3 (d3 eve — replaces inactive roommate_moment)
--
-- The new roommate_evening_day3 shares its title ("The Ordinary Evening") with the
-- deprecated roommate_moment by design; roommate_moment stays inactive below.

BEGIN;

-- ══════════════════════════════════════════════════════════════════════
-- 0. Confirm roommate_moment is deactivated (already inactive; idempotent)
-- ══════════════════════════════════════════════════════════════════════

UPDATE public.storylets
SET is_active = false
WHERE storylet_key = 'roommate_moment';


-- ══════════════════════════════════════════════════════════════════════
-- STORYLET 1 — western_civ_day1 (academic, Day 2 morning)
-- ══════════════════════════════════════════════════════════════════════

INSERT INTO public.storylets (
  slug, title, body, choices, nodes,
  tags, requirements, weight, is_active, is_conflict,
  introduces_npc,
  arc_id, track_id,
...
VALUES (
  ...
  $body$The building smells like floor wax and chalk. Whitmore 203 is a lecture hall — tiered wooden seats that fold up with a thunk, a long blackboard at the front, fluorescent lights that turn everyone slightly green. The clock above the door is three minutes fast.
...
The professor arrives exactly on time with a briefcase and a thermos. He writes "WESTERN CIVILIZATION: AN INTRODUCTION" on the board in capital letters, turns around, and looks at the room like he's counting.$body$,
  ...
  ARRAY['arc_one', 'academic', 'day2', 'class'],
  '{}'::jsonb,
  100, true, false,
  ARRAY['npc_prof_western_civ']::text[],

  (SELECT id FROM public.tracks WHERE key = 'academic'),
  (SELECT id FROM public.tracks WHERE key = 'academic'),

  'western_civ_day1', 'western_civ_day1',
...
-- ══════════════════════════════════════════════════════════════════════
-- STORYLET 2 — bookstore_line (money, Day 2 afternoon)
-- ══════════════════════════════════════════════════════════════════════
...
VALUES (
  ...
  $body$The bookstore line starts at the register and ends somewhere outside. Forty, fifty people deep, snaking through aisles of sweatshirts and coffee mugs with the Harwick crest. The textbook section is in the back — cardboard boxes stacked on metal shelves, organized by department and course number.
...
Your checking account has what your parents deposited in August. You haven't done the math yet on how long it needs to last.$body$,
...
-- ══════════════════════════════════════════════════════════════════════
-- STORYLET 3 — floor_lunch_day2 (belonging, Day 2 afternoon)
-- ══════════════════════════════════════════════════════════════════════
...
VALUES (
  ...
  $body$The dining hall, day two. You know where the trays are now. You know the coffee is bad. You know the brownies are the safest dessert. Small knowledge, but it makes the room feel less foreign.

Doug waves you over. He's already eating, tray loaded — two plates, a glass of milk, a glass of juice. Keith is across from him, working through a sandwich he built from the salad bar fixings. Mike is reading a paperback with one hand and eating with the other. Scott is at the end, quiet, methodical.$body$,
  ...
  'floor_lunch_day2', 'floor_lunch_day2',
...
-- ══════════════════════════════════════════════════════════════════════
-- STORYLET 4 — reading_or_lounge (academic, Day 2 evening, collision)
-- ══════════════════════════════════════════════════════════════════════
...
VALUES (
  ...
  $body$The room is quiet. Scott is at his desk — headphones on, the cord trailing to the cassette player. Through the window the campus is turning blue in the early dark.
...
Down the hall, you can hear the lounge TV and people's voices folding over each other. Someone laughs. The floor is gathering in there without deciding to — it just happens, the way water finds its level.$body$,
  ...
  ARRAY['arc_one', 'academic', 'belonging', 'day2', 'collision'],
  '{}'::jsonb,
  100, true, true,
  ARRAY[]::text[],
  ...
  'reading_or_lounge', 'reading_or_lounge',
...
-- ══════════════════════════════════════════════════════════════════════
-- STORYLET 5 — second_morning_class (academic, Day 3 morning)
-- ══════════════════════════════════════════════════════════════════════
...
Then she turns around and waits.$body$,
  ...
  ARRAY['arc_one', 'academic', 'day3', 'class', 'consequence'],
  '{}'::jsonb,
  100, true, false,
  ARRAY[]::text[],
  ...
  'second_morning_class', 'second_morning_class',
...
-- ══════════════════════════════════════════════════════════════════════
-- STORYLET 6 — hallway_morning_day3 (belonging, Day 3 morning, texture)
-- ══════════════════════════════════════════════════════════════════════
...
VALUES (
  'hallway_morning_day3',
  'Third Floor',

  $body$The hallway, 8:15 AM. Doors opening and closing. Someone's shower is still running. The bulletin board by the stairs has a sign-up sheet for intramural volleyball that nobody has signed.$body$,
  ...
  ARRAY['arc_one', 'belonging', 'day3', 'texture'],
  '{}'::jsonb,
  100, true, false,
  ARRAY[]::text[],
  ...
  'hallway_morning_day3', 'hallway_morning_day3',
...
-- ══════════════════════════════════════════════════════════════════════
-- STORYLET 7 — study_group_forming (academic, Day 3 afternoon)
-- ══════════════════════════════════════════════════════════════════════
...
VALUES (
  ...
  $body$The library is new to you. Four floors of stacks, study carrels along the windows, the particular silence of a building where everyone is pretending to work. Second floor, the tables near the periodicals. Three students from Western Civ have pushed two tables together and spread out their notes.

One of them looks up when you approach. "You're in Moretti's nine o'clock?" Not unfriendly. Assessing.$body$,
  ...
  ARRAY['arc_one', 'academic', 'day3', 'study_group'],
  '{}'::jsonb,
  100, true, false,
  ARRAY[]::text[],
  ...
  'study_group_forming', 'study_group_forming',
...
-- ══════════════════════════════════════════════════════════════════════
-- STORYLET 8 — miguel_afternoon_day3 (belonging, Day 3 afternoon, texture)
-- ══════════════════════════════════════════════════════════════════════
...
VALUES (
  'miguel_afternoon_day3',
  'The Map',

  $body$Miguel is in the common room with a campus map unfolded on the table, the kind they hand out at orientation — glossy, with tiny numbers keyed to a legend on the back. He's tracing a route with his finger and frowning.$body$,
  ...
  ARRAY['arc_one', 'belonging', 'day3', 'texture', 'miguel'],
  '{}'::jsonb,
  100, true, false,
  ARRAY[]::text[],
  ...
  'miguel_afternoon_day3', 'miguel_afternoon_day3',
...
-- ══════════════════════════════════════════════════════════════════════
-- STORYLET 9 — roommate_evening_day3 (roommate, Day 3 evening)
-- Replaces the deprecated `roommate_moment` (same title, different slug).
-- ══════════════════════════════════════════════════════════════════════
...
VALUES (
  'roommate_evening_day3',
  'The Ordinary Evening',

  $body$Scott is at his desk. Not studying — writing something on a legal pad with the concentrated posture of someone who doesn't want to be asked about it. The cassette player is off. The room is quiet in the way rooms are quiet when someone is doing something private in a shared space.

His desk lamp makes a circle of warm light. Everything outside it is dimmer.$body$,
  ...
  ARRAY['arc_one', 'roommate', 'day3', 'scott'],
  '{}'::jsonb,
  100, true, false,
  ARRAY[]::text[],
  ...
  'roommate_evening_day3', 'roommate_evening_day3',
...
-- ══════════════════════════════════════════════════════════════════════
-- STORYLET 10 — catch_up_or_coast (academic, Day 3 evening)
-- Only fires if player skipped reading on Day 2 evening.
-- ══════════════════════════════════════════════════════════════════════
...
VALUES (
  ...
  $body$The Western Civ reader is still on your desk. Page one. The bookmark someone else left in it — a receipt from a gas station in a state you've never been to — marks a spot you haven't reached.

Today in class, you felt the gap. Not a chasm. A step. The kind you can still close if you sit down tonight.

The lounge is audible through the wall.$body$,
  ...
  ARRAY['arc_one', 'academic', 'day3', 'collision'],
  '{"requires_flag": "skipped_reading"}'::jsonb,
  100, true, true,
  ARRAY[]::text[],
  ...
  'catch_up_or_coast', 'catch_up_or_coast',
...
-- ══════════════════════════════════════════════════════════════════════
-- Verification — all 10 inserted + no Dana refs + roommate_moment inactive
-- ══════════════════════════════════════════════════════════════════════
DO $$
DECLARE
  inserted_count integer;
  roommate_moment_active boolean;
BEGIN
  SELECT COUNT(*) INTO inserted_count
  FROM public.storylets
  WHERE storylet_key IN (
    'western_civ_day1', 'bookstore_line', 'floor_lunch_day2',
    'reading_or_lounge', 'second_morning_class', 'hallway_morning_day3',
    'study_group_forming', 'miguel_afternoon_day3', 'roommate_evening_day3',
    'catch_up_or_coast'
  );

  IF inserted_count <> 10 THEN
    RAISE EXCEPTION 'Expected 10 new storylets, found %', inserted_count;
  END IF;

  SELECT is_active INTO roommate_moment_active
  FROM public.storylets WHERE storylet_key = 'roommate_moment';

  IF roommate_moment_active THEN
    RAISE EXCEPTION 'roommate_moment should be inactive after this migration';
  END IF;
END $$;
